...
def check_assumptions(X, y, y_pred, residuals):
    ...
    st.subheader('Ajuste del Modelo de Regresión ElasticNet')
    fig, ax = plt.subplots(figsize=(10, 6))
...
# Función para calcular la regresión ElasticNet
def calculate_elasticnet(X, y, alpha=1.0, l1_ratio=0.5):
...
def show_procedure(X, y, model, r2, mse):
    ...
    st.write("1. Ecuación del modelo de regresión ElasticNet:")
...
def main():
    st.title('Aplicación de Regresión Lineal ElasticNet')
    st.write("Esta aplicación realiza un análisis de regresión lineal múltiple utilizando ElasticNet, incluyendo visualizaciones e interpretaciones detalladas.")
    ...
    if uploaded_file is not None:
        ...
        if df is not None:
            ...
            if len(x_columns) > 0 and y_column:
                ...
                # Parámetro alpha de ElasticNet
                alpha = st.slider('Selecciona el valor de alpha para ElasticNet', min_value=0.01, max_value=10.0, value=1.0, step=0.01)
                l1_ratio = st.slider('Selecciona el valor de l1_ratio para ElasticNet', min_value=0.0, max_value=1.0, value=0.5, step=0.01)
            
                # Cálculo de la regresión ElasticNet
                model, y_pred, residuals, r2, mse = calculate_elasticnet(X, y, alpha, l1_ratio)
                
                # Resultados de la regresión
                st.subheader('Resultados de la Regresión ElasticNet')
                ...
                st.subheader('Verificación de supuestos de la regresión ElasticNet')
