...
def check_assumptions(X, y, y_pred, residuals):
    ...
    st.subheader('Ajuste del Modelo de Regresión Lasso')
    fig, ax = plt.subplots(figsize=(10, 6))
...
# Función para calcular la regresión Lasso
def calculate_lasso(X, y, alpha=1.0):
...
def show_procedure(X, y, model, r2, mse):
    ...
    st.write("1. Ecuación del modelo de regresión Lasso:")
...
def main():
    st.title('Aplicación de Regresión Lineal Lasso')
    st.write("Esta aplicación realiza un análisis de regresión lineal múltiple utilizando Lasso, incluyendo visualizaciones e interpretaciones detalladas.")
    ...
    if uploaded_file is not None:
        ...
        if df is not None:
            ...
            if len(x_columns) > 0 and y_column:
                ...
                # Parámetro alpha de Lasso
                alpha = st.slider('Selecciona el valor de alpha para Lasso', min_value=0.01, max_value=10.0, value=1.0, step=0.01)
            
                # Cálculo de la regresión Lasso
                model, y_pred, residuals, r2, mse = calculate_lasso(X, y, alpha)
                
                # Resultados de la regresión
                st.subheader('Resultados de la Regresión Lasso')
                ...
                st.subheader('Verificación de supuestos de la regresión Lasso')
